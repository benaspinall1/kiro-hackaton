...
function buildPlaceholderPattern() {
    const escaped = Object.values(types_1.PLACEHOLDER_MAP).map(escapeRegExp);
 ...
 */
function overlapsSpan(start, end, spans) {
    return spans.some((span) => start < span.end && end > span.start);
}
/**
 * Find all spans matching a pattern in the text.
 */
function findSpans(text, pattern) {
    const spans = [];
    pattern.lastIndex = 0;
    let match;
    ...
    try {
        ...
        for (const { type, pattern } of PATTERNS) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                const isOverlapping = entities.some((e) => e.type === type &&
